...
module.exports = function auth( options ) {
  ...
  seneca.add({role:plugin,cmd:'is_using_bearer_token'}, cmd_is_using_bearer_token)
  seneca.add({role:plugin,cmd:'load_user_by_bearer_token'}, cmd_load_user_by_bearer_token)
  ...
  // If using local scheme with sessions

  if (options.service.local) {
  ...
  function cmd_is_using_bearer_token (args, done) {

    var using_bearer = options.service.bearer ? true : false
    done(null,{bearer:using_bearer})

  }

  function cmd_load_user_by_bearer_token (args, done) {

    var req = args.req$
    var res = args.res$
    var user = null

    pp_auth.bearer(req, res, function(err) {
      if (err) return done(err);

      if ( req && req.seneca ) {
        req.seneca.user = req.user && req.user.user
  ...
  function buildservice() {
    ...
    _.each( options.service, function(conf,service){
      ...
      pp_auth[service] = function(req,res,next){
        ...
        if (service != 'local' && service != 'bearer' ) {
